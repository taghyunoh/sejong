...
- **조치**(foodMain.jsp `initFoodAutosuggest`) :
  ...
  · Esc 는 종전대로.
- ★★[2026-08-21 「검색 후 스크롤이 안 됩니다」] **목록 안 스크롤이 목록을 닫고 있었다** — 두 화면 공통 2건 :
  ①`scroll` 닫기를 window capture 로 걸면 **목록 자체를 굴리는 스크롤에도 깨어난다**(scroll 은 버블 안 하지만
  capture 는 받는다) → 검색 결과를 굴리려는 순간 닫힘. ⇒ `e.target` 이 목록(식사 listbox / 운동 select) 안이면
  무시 — 바깥(페이지) 스크롤만 닫는다. ②「✓ 입력내용 적용」(sticky 첫 줄)이 **touchstart 즉시 발동**이라
  그 줄에 손을 대고 굴리기 시작하면 적용·닫힘. ⇒ touchstart 는 좌표만 적고 **touchend 에서 이동 10px 미만(탭)일 때만**
  발동(passive:true 라 스크롤도 안 막는다). ⚠「굴리면 닫기」류를 만들 땐 ***안쪽 스크롤 예외 + sticky 단추 탭 판별***을
  같이 넣을 것 — 키보드 스크롤 0.8초 무시(08-20)와 함께 3종 세트다.
...
- ★★**[모바일] 「눌렀더니 떴다가 바로 사라짐」 = 키보드가 올라오며 생기는 스크롤이 닫은 것**
  (2026-08-20 신고). 칸을 누르면 **키보드가 뜨면서 화면이 스크롤**되고, 그 scroll 이
  '굴리면 닫기' 를 깨워 ***방금 연 목록을 즉시 닫는다.***
  ⇒ **연 뒤 0.8초 안의 스크롤은 무시**한다(`openedAt`). 식사·운동 **두 화면 모두** 같은 처리를 넣었다.
  ***연속혈당 달력(ui-datenav.js)에서 겪은 것과 똑같은 함정*** — 목록·달력류를 새로 만들 때 반드시 확인할 것.
...
## ✅[2026-08-21] PWA — 갤럭시 시스템 내비 바가 하단을 차지하는 문제 → `display: fullscreen`

- **증상**(사용자, 스크린샷) : 설치형 PWA 로 열어도 하단에 안드로이드 시스템 바(Ⅲ ◯ ˅)가 **항상 한 층을
  차지**해 우리 하단 메뉴와 두 겹이 됐다. 「PWA 유지하면서」가 조건.
- **조치** : `manifest.webmanifest` 의 `display` **standalone → fullscreen** 한 줄.
  설치형 PWA 로 열면 시스템 바(상단 상태바 포함)가 숨고 화면 전체를 앱이 쓴다 —
  가장자리를 쓸면 잠깐 나타났다 다시 숨는다. **미지원 브라우저는 표준 폴백 체인**
  (fullscreen→standalone)이라 안전. 일반 브라우저 탭으로 열 때는 아무 변화 없다.
- ⚠**트레이드오프 : 상단 상태바(시계·배터리)도 함께 숨는다** — 사용자가 상태바는 남기길 원하면
  이 방식으로는 안 되고(웹 표준에 하단만 숨기는 옵션이 없다) APK 래퍼에서 몰입 모드를 써야 한다.
  되돌리기 = display 를 standalone 으로 한 줄.
- ⚠**기존 설치 사용자에게는 바로 안 바뀐다** — 크롬이 manifest 변경을 감지해 WebAPK 를 갱신하는 데
  하루쯤 걸린다. 즉시 확인 = 홈화면 아이콘 삭제 후 재설치. 운영 반영은 서버에 manifest 파일 교체가 필요.
- 이 PC 에 **안드로이드 래퍼 앱(APK) 소스는 없다**(전체 검색 실측 — D:/세종자료에는 설치 QR·문서뿐).
  APK 경로가 필요해지면 개발사에서 소스를 받아야 한다(몰입 모드 스니펫은 이 날 대화에 정리해 둠).

## ✅[2026-08-21] AI 종합분석 — 날짜 짤림(갤럭시 큰글씨) 근본 해결 + 지표 표기 정리

- **날짜 짤림** : 08-18 두 차례 보정(아이콘 숨김·appearance:none·패딩 축소) 후에도 **갤럭시 + 큰글씨
  설정**에서 「2026. 08. 2」로 또 잘렸다(스크린샷 신고). ⇒ ***date input 의 브라우저 표시를 포기***했다 —
  `input[type=date]` 는 `display:none` 으로 **값 보관용**만 남기고, 표시는 우리가 그리는
  `.date-txt` 텍스트(「2026.08.21」 공백 없는 점 표기, nowrap)로. 이 두 칸은 readonly(이동은 ◀▶)라
  피커가 필요 없어 가능한 방식. 값 세터는 `setDateInputs` **한 곳**이라 동기화도 거기서만 한다.
  `.date-txt` 를 삼성 글자 부스팅 방지 목록(text-size-adjust:100%)에 추가 — 큰글씨 설정에서도 안 커진다.
  ⚠date input 짤림을 CSS 로 계속 쫓지 말 것 — 기기·배율 조합이 끝이 없다. 표시 분리가 답이었다.
- **지표 표기**(사용자 지적 「TIR·TAR·TBR 명확하게 + ↑↓ 가 이상/이하/미만인지 헷갈림」) :
  · 생활습관 가이드 줄 : 「TIR(목표유지) 93% — 권장 70%↑ 충족」 → **「TIR(목표혈당 유지시간) 93% → 충족」**
    (권장 기준은 바로 위 지표 목록의 「권장 : 70% 이상」 힌트가 이미 보여 줘서 화살표 표기를 뺐다).
  · **챗봇도 전부 같은 형식**(같은 날 항목3 지시) : 첫 인사 줄(1847대 — 처음에 빠뜨렸던 세 번째 위치)과
    지표 낱개 답변 줄 모두 **「TIR(목표혈당 유지시간) 93% → 충족」** — 권장 문구·화살표 기호 없이 값 → 판정만.
    한때 「— 권장 70% 이상 →」 병기로 냈다가 사용자 예시 형식대로 단순화했다. CV=혈당변동성.
    ★지표 줄을 만드는 곳이 **세 곳**(가이드 wkAi · 챗봇 첫 인사 · 챗봇 낱개 답변)이다 — 표기를 바꿀 땐 다 같이.
- 단위검증 : setDateInputs 가짜 DOM 실행 → 표시 「2026.08.15 ~ 2026.08.21」·값 yyyy-MM-dd 정상.
  인라인 스크립트 문법검사 — block2 실패는 **HEAD 에도 있는 검사기 한계**(원래 코드의 이스케이프 문자열)로 무관 확인.
- **JSP 만** — target 사본 + tmp4 배포본 복사 완료.

## ✅[2026-08-21] 홈 혈당상태 카드 — 판정 기준 「과거 일주일」 → **「최근 24시간」** + 문구 수정

- 이력 : 오늘 하루(초기) → 과거 일주일(2026-08-05 검토회의) → **최근 24시간(2026-08-21 사용자 지시)**.
- `main.jsp` : `_weekTir`(7일, 일 단위 00:00~23:59) → **`_dayTir`(시각까지 넣은 굴러가는 24시간 창)**.
  Case1 문구 「최근 24시간 동안 발생한 혈당지표 상태입니다.」 /
  Case2(24시간 내 측정 없음) = **마지막 측정 시각으로 끝나는 24시간**으로 대체 계산,
  문구 「<마지막 측정일> 이후 발생한 혈당 측정값이 없습니다 (마지막 측정 24시간 기준 상태입니다)」.
  getLastBloodDate 응답에 시각이 있으면 그 시각, 날짜뿐이면 23:59:59 를 창의 끝으로.
- TIR 70% 판정·정상/관리필요 색은 무변경. Node 단위검증(24h 창 경계·시각 파싱 4경우) 통과.
- **JSP 만** — target 사본 + **이 PC 배포본(D:/egv …/tmp4/wtpwebapps/Sejong_APP)** 복사 완료.
  ★이 PC(계정 user)의 Eclipse 워크스페이스는 C:/Users/HYUN 이 아니라 **D:/egv** 다(tmp4=Sejong_APP).
...
## ★[2026-08-25] 연속혈당 화살표 — 30분 기준으로 옮겼다가 **같은 날 저녁 5분 기준으로 되돌림**

> ***화살표 근거를 또 바꾸자는 얘기가 나오면 이 항목부터 볼 것.*** 하루에 두 번 왕복했다.

- **오전(11:06)**: 요청 「기기 설명서의 변화 속도 화살표 표 그대로」 → `applyTrend30()` 신설.
  판정 기준 = **지난 30분 변화량**(≤30 안정적 · 31~60 서서히 · 61~90 · ≥91 빠르게), 30분 전 값은
  차트가 이미 받아 둔 시리즈에서 ±10분 근사로 찾는다(서버 무변경). **상태·설명문·화살표를 모두** 여기서 그렸다.
- **저녁(18:44 실화면 지적 「화살표 방향 수정해줘」)**: 화면 캡처 = `165(6:35) ↗ -2 mg/dL/5분 163(6:40)`.
  ***버그가 아니라 근거가 둘이라서 생긴 어긋남이다*** — 30분 동안은 올랐고(↗) 마지막 5분만 내려간(-2) 상황.
  ⇒ **상단 화살표만 5분 기준으로 복귀**(2026-08-20 의 연속 기울임 그대로 · `_RATE_FULL=2`).
- ★**왜 화살표는 5분이어야 하나** — 마크업이 `[화살표][숫자]` + 아래 `mg/dL/5분` 로 **한 덩어리**이고,
  그 좌우가 5분 간격 두 측정값(165 · 163)이다. 이 덩어리 안에서 화살표만 다른 시간축을 쓰면
  **읽는 사람에겐 그냥 반대 방향**이다. 30분 기준을 상단에 두려면 숫자·단위·좌우 값까지 다 바꿔야 한다.
- **지금 구조 (근거가 둘, 자리가 다르다)** :
  · **상단 화살표 + 숫자** = 직전 5분 (`showBloodData` 안, 색도 `_dirColor` 하나로 공유)
  · **하단 '현재 혈당 변화' 상태·설명문** = 설명서의 지난 30분 (`applyTrend30` — 이제 **화살표를 안 건드린다**)
  ⚠두 곳이 같은 요소를 칠하지 않으므로 조회 응답 순서(showBloodData ↔ 차트)와 무관하다. **다시 합치지 말 것.**
- 계산 불가(직전 측정 없음)일 때 화살표는 `=` 회색 — 수평 `→` 는 '안정적'으로 읽힌다.
- **JSP 만**(`FAHR_00.jsp`) — target 사본 + Eclipse 배포본(`…/tmp5/wtpwebapps/Sejong_APP`) 복사 완료.
- ★**최종형(같은 날 저녁 2차 지적 「직관적으로 · 사용자한테 설명해야 하는 변수는 배제」)** :
  화살표 근거를 **화면에 보이는 두 수치의 차이 하나로** 줄였다. 분당 변화율(mg/dL/분)·30분 추세처럼
  ***화면에 없는 값은 쓰지 않는다*** — 보고 있는 숫자가 곧 화살표라 따로 설명할 것이 없다.
  · 각도 = `-(point/ARROW_FULL)*90` (`ARROW_FULL=10` = 5분에 ±10 이면 수직). 오르면 위·내리면 아래·같으면 수평.
  · 툴팁도 말글로 : 「5분 전보다 2 낮아졌습니다」. 종전 「분당 -0.4 mg/dL」 같은 표기는 뺐다.
  · ⚠기울기를 조절할 일이 생기면 **`ARROW_FULL` 하나만** 고칠 것.

## ★[2026-08-25] AI 종합분석·챗봇 — ※ 각주 글자 키움 (12px → 14px)

- **요청**: 실화면 캡처에 「※ 64세 · 당뇨병 전단계 — 일반 기준 적용 (TIR 70% 이상 …)」 줄을 표시하며
  *"표시글자 폰트 크게, AI 챗봇도 같이 변경"*. 본문이 15px 인데 이 줄만 12px 라 안 읽혔다.
- **조치 2곳**(`Blood_Consult.jsp`) :
  · `_whoTxt`(적용 기준 줄) 인라인 **12 → 14px** + 줄간격 1.55 · 색 `#6b7889 → #5b6a7d`.
  · 말풍선 안 `<small>※ …</small>` (기본 0.8em = 12px) → **13.5px** (`.chat-bot small, .chat-user small`).
- ★**챗봇은 저절로 따라온다** — `_chatIntroAnalysis`·`_chatStatusSummary` 가 **`#wkCoach` 의 innerHTML 을
  그대로 가져다** 말풍선에 넣기 때문. ***그래서 이 줄의 서식을 고칠 때는 한 곳만 고치면 되고,
  반대로 #wkCoach 구조를 바꾸면 챗봇 말풍선이 함께 깨진다.***
- **JSP 만** — target 사본 + Eclipse 배포본 복사 완료. CSS 는 인라인·`<style>` 이라 Ctrl+F5.
- ★**이어서 「표시부분 제거」(같은 날 저녁, AI 챗봇도 함께)** — 두 번째 ※ 줄(`_STD.note`,
  「나이를 고려해 저혈당을 더 엄격히, 고혈당은 조금 느슨하게 봅니다」)을 **화면에서 뺐다.**
  바로 윗줄이 이미 적용 수치(TIR 50% 이상 · TAR 50% 미만 · TBR 1% 미만)를 밝히므로 같은 말이 겹쳤다.
  · 고친 곳은 `_whoTxt` **한 줄** — 챗봇은 `#wkCoach` 를 그대로 가져다 쓰므로 **함께 사라진다**.
  · ⚠서버 `CgmTarget.note`·`${stdNote}`·`_STD.note` 값 자체는 남겨 두었다(되살릴 땐 그 한 줄만 복구).
  · 말풍선 각주 `<small>※ 대한당뇨병학회 관리지표 기준</small>` 은 **다른 줄이라 그대로 둔다.**

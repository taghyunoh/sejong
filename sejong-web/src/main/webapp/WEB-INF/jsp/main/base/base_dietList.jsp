...
//	 }
	$.ajax({
	   	url : CommonUtil.getContextPath() + 'base/ctl_dietList.do',
	    type : 'post',
	    data: {diet_gb: $("#searchText").val() },
	   	dataType : "json",
	   		...
	   		if(data.resultCnt > 0 ){
	    		...
	    		for(var i=0 ; i < data.resultCnt; i++){
	    			dataTxt = '<tr class="" onclick="javascript:fnDtlSearch(\'' 
    		            ...
    		            + data.resultLst[i].diet_gb   + '\', \''
    		            + data.resultLst[i].start_dt  + '\', \''
    		            + data.resultLst[i].end_dt    + '\');" id="row_'
    		            + data.resultLst[i].hosp_uuid 
    		            + data.resultLst[i].diet_gb 
    		            + data.resultLst[i].start_dt 
    		            + data.resultLst[i].end_dt + '">';
	        
					dataTxt += 	"<td>" + (i+1)  + "</td>" ; 
					dataTxt +=  "<td>" + data.resultLst[i].hosp_nm   + "</td>" ;
					dataTxt +=  "<td>" + data.resultLst[i].diet_gb   + "</td>" ;
...
function fnDtlSearch(hosp_uuid ,diet_gb , start_dt, end_dt){ 
	if (!hosp_uuid || !diet_gb || !start_dt|| !end_dt) return;
	document.regForm.iud.value  = "U";
	document.regForm.hosp_uuid.value  = hosp_uuid ; 
	document.regForm.diet_gb.value    = diet_gb ; 
	document.regForm.start_dt.value   = start_dt ; 
	document.regForm.end_dt.value     = end_dt ;
	
    $("#infoTable tr").removeClass("tr-primary");
    $("#infoTable #row_" + hosp_uuid + diet_gb + start_dt + end_dt).addClass("tr-primary");
}
...
function fnSave(iud){
	...
	if(iud == "I"){
		...
		$("#diet_gb").css("pointer-events", "auto").css("background-color", ""); // 콤보박스 활성화 
		...
		$.ajax( {
			type : "post",
			url : CommonUtil.getContextPath() + "/base/selectDietInfo.do",
			data : {hosp_uuid: $("#hosp_uuid").val() , diet_gb : $("#diet_gb").val(), 
				    start_dt : $("#start_dt").val()  , end_dt : $("#end_dt").val() },
			dataType : "json",
			success : function(data) {    
				if(data.error_code != "0") {
					alert(data.error_msg);
				...
				$("#diet_gb").val(data.result.diet_gb);
				...
				$("#diet_gb").css("pointer-events", "none").css("background-color", "#e9ecef"); // 콤보박스 비활성화 
...
function fnSaveProc(){
	if(!fnRequired('diet_gb', '가산식사코드를 확인하세요.'))  return;
	...
	if(confirm(""+ msg)) {
		$.ajax( {
			type : "post" ,                       
			url : CommonUtil.getContextPath() + "/base/DietSaveProc.do",
			data : formData,
			dataType : "json",
			success : function(data) {    
				if(data.error_code != "0") {
	  	...
	  	<div class="content-wrap"> 
			<div class="flex-left-right mb-10">
				<div class="patient-info">
					<div class="info-name">가산식대 목록</div>
				</div>
			</div>
        ...
        <div class="modal-body">
          <div class="form-container"> 
           ...
           <div class="form-wrap w-80">
	           <label for="">가산식대구분</label> 
	            <select class="form-select" name="diet_gb" id="diet_gb"> <!-- readonly -->
